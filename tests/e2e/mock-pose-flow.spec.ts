...
test("モック姿勢で開始し、結果画面から再プレイ準備へ戻れる", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "体がコントローラー！ ウォール回避ゲーム" }),
  ).toBeVisible();
  ...
  await page.getByLabel("ランキング用ニックネーム").fill("E2Eプレイヤー");
  ...
  const wallCue = page.getByLabel("次の動きと壁のタイミング");
  await expect(wallCue).toBeVisible();
  await expect(wallCue).toContainText("中央でまっすぐ");
  await expect(wallCue.getByRole("progressbar", { name: "壁の接近度" })).toBeVisible();
  await expect(wallCue).toHaveAttribute("data-phase", "act", {
    timeout: 3_000,
  });
  await expect(wallCue).toContainText("いま！");
  await expect(wallCue).toHaveAttribute("data-phase", "preview", {
    timeout: 1_500,
  });
  await expect(wallCue).toContainText("左へ移動");
  ...
  await expect(page.getByText("ニックネーム E2Eプレイヤー")).toBeVisible();
  await expect(page.getByText(/現在 \d+位です/)).toBeVisible();
  await page.getByRole("button", { name: "ランキングを見る" }).click();
  const rankingDialog = page.getByRole("dialog", { name: "ランキング" });
  await expect(rankingDialog).toBeVisible();
  await expect(rankingDialog).toContainText("E2Eプレイヤー");
...
test("展示向けリセットで準備画面とプレイ画面からタイトルへ戻れる", async ({
  ...
  const nicknameInput = page.getByLabel("ランキング用ニックネーム");
  await expect(page.getByRole("button", { name: "ゲーム開始" })).toBeDisabled();
  await nicknameInput.fill("リセット確認");
  await page.getByRole("button", { name: "ゲーム開始" }).click();
  await expect(page.getByRole("heading", { name: "カメラ準備" })).toBeVisible();
  ...
  await expect(page.getByLabel("ランキング用ニックネーム")).toHaveValue("");

  await page.getByLabel("ランキング用ニックネーム").fill("次のプレイヤー");
  await page.getByRole("button", { name: "ゲーム開始" }).click();
  await page.getByRole("button", { name: "モック姿勢で試す" }).click();
  await expect(
    page.getByLabel("プレイヤー姿勢と壁パターンのゲーム描画"),
